...
export default function AdminProjectsPage() {
  ...
  return (
    <div>
      ...
      />
      {search.trim() && (
        <p className="mb-3 text-xs text-white/60">Reordering is disabled while searching — clear the search to drag order.</p>
      )}

      {loading ? (
        <p className="text-white/60">Loading...</p>
      ) : filtered.length === 0 ? (
        <p className="text-white/60">{search.trim() ? "No projects match your search." : "No projects yet."}</p>
      ) : (
