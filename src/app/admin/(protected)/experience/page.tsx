    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        <div className="glass divide-y divide-white/10">
          {items.map((it) => (
            <div key={it.id} className="flex items-center justify-between gap-3 p-4">
              <div>
                ...
                <div className="text-xs text-white/60">{it.startDate?.slice(0, 10)} — {it.current ? "Present" : it.endDate?.slice(0, 10)}</div>
              </div>
          ...
          {items.length === 0 && <p className="p-4 text-white/60">No experience entries yet.</p>}
        </div>
