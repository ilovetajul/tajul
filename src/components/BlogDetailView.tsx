  ...
  return (
    <article className="section-padding container-xl max-w-3xl">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          {post.category && (
            <Link href={`/blog?category=${post.category.slug}`} className="text-sm uppercase tracking-wide text-white/60">
              {post.category.name}
            </Link>
          )}
      ...
      {post.featuredImg && (
        <div className="glass relative my-8 h-72 w-full overflow-hidden">
          <Image src={post.featuredImg} alt={title} fill sizes="(max-width: 768px) 100vw, 768px" className="object-cover" />
        </div>
      )}
