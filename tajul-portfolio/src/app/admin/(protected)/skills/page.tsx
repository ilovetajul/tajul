...
const empty = { name: "", categoryId: "", level: "Intermediate", description: "", yearsExp: "" };
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        <div className="glass divide-y divide-white/10">
          {items.map((it) => (
            <div key={it.id} className="flex items-center justify-between gap-3 p-4">
              <div>
                ...
                <div className="text-xs text-white/40">{it.category?.name || "Uncategorized"} · {it.level}</div>
              </div>
          ...
          {items.length === 0 && <p className="p-4 text-white/40">No skills yet.</p>}
        </div>
